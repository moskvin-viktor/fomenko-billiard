...
Pushing a `v*` tag builds native binaries for Linux, Windows and macOS plus
the web bundle, and attaches all four to that tag's GitHub Release
(`.github/workflows/release.yml`):

```sh
# bump `version` in Cargo.toml to match first
git tag v0.1.0
git push --tags
```

`main` is the development line; a tagged release is the frozen, versioned
snapshot people should actually download.
