...
pub fn classify_level(
    ...
    let (u1, u2) = if lc < b {
        (
            ULength::new(0.0, lc, a, b, lc, RootSide::Hi, 512),
            ULength::new(tab.hyp[0], hyp_max, a, b, lc, RootSide::Hi, 512),
        )
    } else {
        (
            ULength::new(0.0, tab.ell[tab.ell.len() - 1], a, b, lc, RootSide::Hi, 512),
            ULength::new(lc, hyp_max, a, b, lc, RootSide::Lo, 512),
        )
    };
    ...
    let (ell_ref, hyp_ref) = if lc < b {
        (insert_sorted(&tab.ell, lc), tab.hyp.clone())
    } else {
        (tab.ell.clone(), insert_sorted(&tab.hyp, lc))
    };
...
/// Find the original cell `(oi, oj)` containing the point `(λ₁, λ₂)`.
fn containing_cell(tab: &Table, lam1: f32, lam2: f32) -> (usize, usize) {
    let oi = tab
        .ell
